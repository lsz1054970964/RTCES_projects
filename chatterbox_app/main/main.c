...
static void chatterbox_task(void *v_params) {
  ...
  vTaskDelay(params->release_time * mainTASK_CHATTERBOX_OUTPUT_FREQUENCY);
  ...
  if (params->repetitions == -1) {
    ...
    for (;;) {
        ...
        vTaskDelayUntil(&last_wake_time, params->period * mainTASK_CHATTERBOX_OUTPUT_FREQUENCY);
    }
} else {
    TickType_t last_wake_time = xTaskGetTickCount();
    // LED blicks for certain times
...
void app_main(void) {
  ...
  xTaskCreate(chatterbox_task, "Task1", 2048, &task1_params, mainTASK_CHATTERBOX_TASK1_PRIORITY, NULL); 
  xTaskCreate(chatterbox_task, "Task2", 2048, &task2_params, mainTASK_CHATTERBOX_TASK2_PRIORITY, NULL);
  xTaskCreate(chatterbox_task, "Task3", 2048, &task3_params, mainTASK_CHATTERBOX_TASK3_PRIORITY, NULL); 
