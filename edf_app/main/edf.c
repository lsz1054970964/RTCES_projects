...
void system_density_test(PeriodicTaskParams **params, TaskId task_id,
  ...
  for (TaskId i = 0; i <= task_id; i++) {
     ...
     * Recall the adjusted system density test for deferrable servers.
     * That is, you have to implement the following functionality:
     *  - Increment the system density, depending on params[i]->type
     *    (either PERIODIC_TASK or DEFERRABLE_SERVER).
