 ...
 * It is your job to extend this implementation by an deferrable
 * server and the adjusted system density test. To summarize, you should:
 * 1. Complete 'system_density_test' that is located in edf.c.
 * 2. Complete 'deferrable_server_implementation' that is located in tasks.c.
  ...
  for (TaskId i = 0; i < NUMBER_OF_TASKS; i++) {
    ...
    if (task_set[i]->type == PERIODIC_TASK) {
      ...
      xTaskCreate((void *)periodic_task_implementation, task_name,
                  ...
                  &task_set[i]->handle);
    } else if (task_set[i]->type == DEFERRABLE_SERVER) {
      xTaskCreate((void *)deferrable_server_implementation, "DS",
                  configMINIMAL_STACK_SIZE + 256, task_set[i], tskIDLE_PRIORITY,
                  &task_set[i]->handle);
    }
  }
